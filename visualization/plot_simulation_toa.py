...
def plot_simulation_fs(FN, linestyle="solid"):
    ...
    x_data, y_data = load_toa_data(FN)
...
def plot_simulation_s(FN_CRIT, FN_BAND):
    ...
    x_data, y_data_crit = load_toa_data(FN_CRIT)
    _, y_data_band = load_toa_data(FN_BAND)

    x_data_map = x_data <= 16  # Only plot up to 16 samples
    x_data = x_data[x_data_map]
    y_data_crit = y_data_crit[x_data_map, 4:]  # remove other interps
    y_data_band = y_data_band[x_data_map, 4:]  # remove other interps
    ...
    fig.tight_layout()
    fig.set_size_inches(w=text_width, h=text_height)
    fig.savefig("./figures/sim_s_toa.pdf")
    return ax
